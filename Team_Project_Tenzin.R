...
# Select relevant columns for logistic regression model (ADDED TFT)
clean_data_ais_model <- subset(clean_data2, 
                               select=c(Corticoid, Depression, BMI, Liver.diag,
                                        RGD, Any.fibro, Renal.fail, Gender, AIS, TFT))
...
#' Recall there are 151 participants who experience sleep disturbance 
...
# Using stepwise and comparing it with manual model for AIS.
ais_glm_mod_full <- glm(AIS ~., data = clean_data_ais_model, family="binomial")
ais_glm_step_back_new <- stepAIC(ais_glm_mod_full,trace = F)
summary(ais_glm_step_back_new)

anova(ais_glm_mod_6, ais_glm_step_back_new)
